// Schedules.js - Thời khóa biểu với Calendar Zoom In/Out (Năm → Tháng → Tuần)
...
export async function renderSchedules(container) {
  ...
  try {
    ...
    // State cho calendar
    let viewMode = 'week'; // 'year' | 'month' | 'week'
    ...
    function renderYearView() {
      ...
      calEl.innerHTML = `
        <div class="flex justify-between items-center px-6 py-4 border-b border-apple-divider select-none">
          ...
          <span class="text-sm font-bold text-apple-ink">${selectedYear}</span>
        ...
        <div class="px-6 pb-4 text-center">
          <p class="text-[10px] text-slate-400">Chạm vào tháng để xem lịch tuần</p>
        </div>
      `;
    ...
    function renderMonthView() {
      ...
      calEl.innerHTML = `
        <div class="flex justify-between items-center px-6 py-4 border-b border-apple-divider select-none">
          <button id="btn-back-to-year" class="flex items-center gap-1 text-xs text-apple-blue font-semibold hover:underline">
            <span class="material-symbols-outlined text-[16px]">arrow_back_ios</span> ${selectedYear}
          </button>
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm font-bold text-apple-ink">${monthNames[selectedMonth]} ${selectedYear}</span>
            ...
            ${cells.map(date => {
              ...
              return `
                <button type="button" class="day-cell-btn aspect-square rounded-xl flex flex-col items-center justify-center transition p-1
                  ...
                  }" data-date="${dateStr}" data-month="${date.getMonth()}">
        ...
        <div class="px-6 pb-4 text-center">
          <p class="text-[10px] text-slate-400">Chạm vào ngày để xem lịch tuần</p>
        </div>
      `;

      calEl.querySelector('#btn-back-to-year')?.addEventListener('click', () => { viewMode = 'year'; renderCalendar(); });
    ...
    function renderWeekView() {
      ...
      calEl.innerHTML = `
        <div class="flex justify-between items-center px-5 py-3 border-b border-apple-divider select-none flex-wrap gap-2">
          <button id="btn-back-to-month" class="flex items-center gap-1 text-xs text-apple-blue font-semibold hover:underline">
            <span class="material-symbols-outlined text-[16px]">arrow_back_ios</span> Tháng ${selectedMonth + 1}/${selectedYear}
          </button>
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-apple-ink">${startStr} — ${endStr}</span>
      ...
      calEl.querySelector('#btn-back-to-month')?.addEventListener('click', () => {
...
>>>>>>> 0b53f0f445b13ccaab6d05b2c20179af06e02497
    ...
    function updateViewBtns() {
      const btns = { year: 'view-year-btn', month: 'view-month-btn', week: 'view-week-btn' };
