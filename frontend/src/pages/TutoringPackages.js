    ...
    container.innerHTML = `
      <div class="space-y-6">
        <div class="flex justify-between items-center">
          ${role === 'admin' || role === 'le_tan' ? `
            <button id="btn-open-create-tutor-pkg" class="flex items-center gap-1 px-4 py-2 rounded-full bg-apple-blue text-white text-xs font-semibold hover:opacity-90 transition active:scale-95 shadow-sm">
      ...
      <div id="tutor-pkg-modal" class="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center hidden p-4">
        <div class="bg-white rounded-3xl max-w-md w-full p-6 space-y-4 border border-[#e2e2e4] shadow-xl">
          <div class="flex justify-between items-center pb-3 border-b border-[#f3f3f5]">
            <h3 id="tutor-modal-title" class="text-sm font-bold text-[#1a1c1d] flex items-center gap-2">
          ...
          <form id="tutor-pkg-form" class="space-y-4 text-xs">
            ...
            <div class="flex justify-end gap-2 pt-4 border-t border-[#f3f3f5]">
              <button type="button" id="btn-cancel-tutor-modal" class="px-5 py-2.5 rounded-xl border border-[#e2e2e4] hover:bg-slate-50 text-slate-700 font-semibold transition active:scale-95">Hủy bỏ</button>
